...
- 생물 카테고리는 계, 문, 강 태그를 사용하되, 필요한 만큼만 추천하세요 (최대 3개, 억지로 채울 필요 없음)`;
...
다음 JSON 형식으로 정확하게 답변해주세요:
{
  ...
  "tags": ["태그1", "태그2"] 또는 ["태그1"] 또는 []
}

태그 규칙:
- **중요: 태그는 필요한 만큼만 추천하세요. 억지로 채울 필요가 없습니다.**
- 태그가 0개여도 되고, 1개만 있어도 되고, 최대 3개까지 가능합니다.
- 기존 태그 목록(${existingTagsText})을 우선 사용하고, 기존 태그로 표현 가능하면 새로운 태그를 만들지 않음
- 카테고리와 중복 제외, 모순 태그 동시 사용 금지, 핵심 태그만 추천 (최대 3개), 구체적 태그 우선
- "생물" 카테고리인 경우 생물학적 분류 태그(계, 문, 강)를 사용하되, 필요한 만큼만 추천 (최대 3개, 억지로 채울 필요 없음)
...
{
  "approved": true,
  "reason": "웹 검색 결과 실제 존재하는 항목으로 확인되었습니다.",
  "category": null,
  "tags": []
}
